...
export class LaunchBrowser {
  ...
  async init(): Promise<void> {
    ...
    const sessionDir = path.resolve(`session/${this.email}`);
    ...
    const UndetectableBMS = new UndetectableBrowser(
      await puppeteer.launch({
        ...
        userDataDir: `session/${this.email}`,
        args: [
          "--no-sandbox",
    ...
    await this.page.setViewport({ width: 1800, height: 3812 });
    await this.page.setUserAgent(
      "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/132.0.0.0 Safari/537.36"
    );
    setEnv(`SESSION_DIR_${this.email}`, `session/${this.email}`);
  }
